 ...
 */
import { beijingHour, bucketCosts } from "./pricing.js";
...
export function foldEvent(usage, event, estimate) {
    if (typeof event.time === 'number' && event.time > usage.lastAt)
        usage.lastAt = event.time;
    ...
    if (event.type === 'request/header') {
        ...
        if (typeof header?.config?.model === 'string')
            usage.currentModel = header.config.model;
        return;
    }
...
export function foldEvents(usage, events, estimate) {
    for (const event of events)
        foldEvent(usage, event, estimate);
}
export function totalsOf(usage, table, nowMs) {
