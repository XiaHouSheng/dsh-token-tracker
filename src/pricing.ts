...
export function periodOfHour(hour: number, table: PricingTable): 'peak' | 'offpeak' {
  const peaks = Array.isArray(table.peakHours) ? table.peakHours : []
  for (const p of peaks) {
    if (typeof p.start === 'number' && typeof p.end === 'number' && hour >= p.start && hour < p.end) return 'peak'
  }
  return 'offpeak'
}

export function priceEntry(model: string, table: PricingTable, nowMs: number): PricingEntry | undefined {
  ...
  for (const entry of list) {
    ...
    if (!Number.isNaN(from) && from <= nowMs && from > best) { chosen = entry; best = from }
  }
  if (best === -Infinity) {
    let earliestFrom = Infinity
    for (const entry of list) {
      const from = typeof entry.effectiveFrom === 'string' ? Date.parse(entry.effectiveFrom) : Infinity
      if (from < earliestFrom) { earliestFrom = from; chosen = entry }
    }
  }
  return chosen
}

export function priceOf(entry: PricingEntry | undefined, period: 'peak' | 'offpeak', key: 'inputCached' | 'inputUncached' | 'output'): number | undefined {
...
export function bucketCosts(buckets: readonly CostBucket[], table: PricingTable, nowMs: number): { cached: number; uncached: number; output: number; estimated: number; total: number; unpriced: boolean } {
  let cached = 0
  let uncached = 0
  let output = 0
  let estimated = 0
  ...
  for (const bk of buckets) {
    ...
    if (entry === undefined) unpriced = true
