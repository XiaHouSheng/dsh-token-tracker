...
import { DEFAULT_PRICING, beijingHour, periodOfHour, priceEntry, bucketCosts } from './pricing.ts'
import { newUsage, foldEvents, foldEvent, totalsOf, estimateMessage as localEstimate, type UsageState } from './usage.ts'
import type { OverviewPayload, PeriodInfo, PricingEntry, PricingTable, UsageDetail, UsageTotals, UsageTurn } from './types.ts'
...
export class TokenTrackerService extends TypertRemoteService {
  ...
  private estimate(message: unknown): number {
    const meter = this.ctx.get('tokenMeter') as { estimateMessage?: (m: unknown) => number } | undefined
    if (meter?.estimateMessage) {
      try { return meter.estimateMessage(message) } catch { /* fall through */ }
    }
  ...
  private async candidatePaths(): Promise<string[]> {
    const paths: string[] = []
    const policy = this.ctx.get('sandboxPolicy') as { workspaceRoot?: string } | undefined
    if (typeof policy?.workspaceRoot === 'string') paths.push(policy.workspaceRoot)
    const sessionQuery = this.ctx.get('sessionQuery') as { listSessions?: () => Promise<{ header?: { cwd?: string } }[]> } | undefined
  ...
  private async loadPricing(): Promise<{ table: PricingTable; source: 'local' | 'file' | 'default' }> {
    const now = Date.now()
    ...
    const fs = this.ctx.get('fs') as { resolve?: (p: string) => Promise<unknown>; readText?: (t: unknown) => Promise<string> } | undefined
  ...
  private async buildOverview(pricing: { table: PricingTable; source: 'local' | 'file' | 'default' }): Promise<OverviewPayload> {
    const sessionQuery = this.ctx.get('sessionQuery') as {
      listSessions?: () => Promise<{ header?: { id?: string; cwd?: string; createdAt?: number }; live?: boolean }[]>
      readTitleSnapshots?: (ids: string[]) => Promise<{ sessionId: string; status: string; value?: { title?: { title?: string } } }[]>
    } | undefined
    let records: { header?: { id?: string; cwd?: string; createdAt?: number }; live?: boolean }[] = []
    if (sessionQuery?.listSessions) {
      try { records = await sessionQuery.listSessions() } catch { records = [] }
    }
    const ids: string[] = []
    for (const rec of records) {
    ...
    if (sessionQuery?.readTitleSnapshots && ids.length > 0) {
      try {
        ...
        for (const obs of observations) {
          if (obs?.status === 'fulfilled' && obs.value?.title?.title) titleMap.set(obs.sessionId, obs.value.title.title)
        }
      } catch { /* ignore */ }
    }
    ...
    for (const s of sessions) {
      if (s.live && s.lastActiveAt > bestAt) { bestAt = s.lastActiveAt; currentSessionId = s.sessionId }
    }
  ...
  @Remote('turn')
  async usageTurn(args: { sessionId: string; turn: number }): Promise<UsageTurn | null> {
    const pricing = await this.loadPricing()
    const entry = await this.getUsage(args.sessionId)
    if (!entry) return null
  ...
  @Remote('setPricing')
  async setPricing(args: { json: string | null }): Promise<{ ok: boolean; error?: string }> {
  ...
  private async serveOverviewApi(req: IncomingMessage, res: ServerResponse): Promise<void> {
    const url = req.url ?? ''
    ...
    try {
      ...
      if (sessionId) {
        ...
        if (!entry) { this.sendJson(res, 404, { error: 'session not found' }); return }
        const totals = totalsOf(entry.usage, pricing.table, Date.now())
  ...
  private sendJson(res: ServerResponse, status: number, payload: unknown): void {
    let body: string
    try { body = JSON.stringify(payload) } catch { body = '{"error":"serialize failed"}' }
